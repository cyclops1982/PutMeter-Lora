...
SensorHelper::MeasurementResult SensorHelper::PerformMeasurement(uint offsetInMM, uint maxdepthInMM)
{
  ...
  fs::File file = LittleFS.open(caliberationfilename, "rb", false);
  if(file) {
    VL53L4CX_CalibrationData_t calData;
    file.readBytes((char *)&calData, sizeof(calData));
    Logger::debug("Read %d bytes...", sizeof(calData));
    status = sensor_vl53l4cx_sat.VL53L4CX_SetCalibrationData(&calData);
    Logger::debug("VL53L4CX_SetCalibrationData status: %s", SensorHelper::ErrorToString(status));
  ...
  Logger::debug("VL53L4CX_SetXTalkCompensationEnable status: %s", SensorHelper::ErrorToString(status));
  if(status != VL53L4CX_ERROR_NONE) {
    result.StatusText = ErrorToString(status);
    result.StatusCode = -3;
    return result;
  ...
  Logger::debug("VL53L4CX_SetMeasurementTimingBudgetMicroSeconds status: %s", SensorHelper::ErrorToString(status));
  if(status != VL53L4CX_ERROR_NONE) {
    result.StatusText = ErrorToString(status);
    result.StatusCode = -4;
    return result;
  ...
  Logger::debug("VL53L4CX_SetDistanceMode status: %s", SensorHelper::ErrorToString(status));
  if(status != VL53L4CX_ERROR_NONE) {
    result.StatusText = ErrorToString(status);
    result.StatusCode = -5;
    return result;
  ...
  Logger::debug("VL53L4CX_SetOffsetCorrectionMode status: %s", SensorHelper::ErrorToString(status));
  if(status != VL53L4CX_ERROR_NONE) {
    result.StatusText = ErrorToString(status);
    result.StatusCode = -6;
    return result;
  ...
  Logger::debug("VL53L4CX_SmudgeCorrectionEnable status: %s", SensorHelper::ErrorToString(status));
  if(status != VL53L4CX_ERROR_NONE) {
    result.StatusText = ErrorToString(status);
    result.StatusCode = -7;
    return result;
  ...
  Logger::debug("Started measurement status: %s", SensorHelper::ErrorToString(status));
  if(status != VL53L4CX_ERROR_NONE) {
    result.StatusText = ErrorToString(status);
    result.StatusCode = -8;
    return result;
  ...
  for(; result.NrOfMeasurements < 5; result.NrOfMeasurements++) {
    do {
      ...
      Logger::debug("%d: Status of data: %d; NewData: %u", result.NrOfMeasurements, status, NewDataReady);
      delay(100);
    } while(!NewDataReady);
    ...
    Logger::debug("%d: Status read: %d/%s", result.NrOfMeasurements, status, SensorHelper::ErrorToString(status));
    Logger::debug("%d: VL53L4CX Satellite: Count=%d, #Objs=%1d ", result.NrOfMeasurements, pMultiRangingData->StreamCount,
    ...
    for(int j = 0; j < pMultiRangingData->NumberOfObjectsFound; j++) {
      Logger::debug("  %d: status=%s; Distance: %dmm", j, SensorHelper::ErrorToString(pMultiRangingData->RangeData[j].RangeStatus),
                    pMultiRangingData->RangeData[j].RangeMilliMeter);
      Logger::debug("  %d: Signal=%f Mcps;Ambient=%f Mcps ", j, (float)pMultiRangingData->RangeData[j].SignalRateRtnMegaCps / 65536.0,
  ...
  Logger::debug("RESULT = %d", result.DistanceInMM);
  return result;
}
...
bool SensorHelper::Init()
{
  if(!initialized) {
    ...
    bool wireresult = Wire.begin(SDA, SCL);
    Logger::debug("Wire.begin(SDA, SCL) result: %d", wireresult);
    if(!wireresult) {
      return false;
    }
    delay(100);
    int res = sensor_vl53l4cx_sat.begin();
    Logger::debug("sensor.begin(): %d", res);
    if(res != 0) {
      return false;
    }
    delay(100);
    VL53L4CX_Error status = sensor_vl53l4cx_sat.InitSensor(0x12);
    Logger::debug("InitSensor status: %s", SensorHelper::ErrorToString(status));
    if(status != VL53L4CX_ERROR_NONE) {
      return false;
    }
...
bool SensorHelper::StoreCalData()
{
  ...
  Logger::debug("StoreCalData()  - VL53L4CX_GetCalibrationData status: %s", SensorHelper::ErrorToString(status));

  if(LittleFS.exists(caliberationfilename)) {
    Logger::debug("StoreCalData() - Existing caliberation file exists, removing!");
    LittleFS.remove(caliberationfilename);
  }
  fs::File file = LittleFS.open(SensorHelper::caliberationfilename, "wb", true);
  if(!file) {
    Logger::error("StoreCalData() - FAILED TO OPEN FILE!: %d", file);
    return false;
  }

  auto size = file.write((uint8_t *)calData, sizeof(*calData));
  file.close();
  Logger::debug("StoreCalData() - Writen %d data. exiting", size);
  return true;
}
...
bool SensorHelper::LoadCalData()
{
  ...
  if(file) {
    VL53L4CX_CalibrationData_t calData;
    file.readBytes((char *)&calData, sizeof(calData));
    Logger::debug("SetCalData() - Read %d bytes...", sizeof(calData));
    VL53L4CX_Error status = sensor_vl53l4cx_sat.VL53L4CX_SetCalibrationData(&calData);
    Logger::debug("SetCalData() - VL53L4CX_SetCalibrationData status: %s", SensorHelper::ErrorToString(status));
    file.close();
    return true;
  }
  Logger::debug("SetCalData() - failed, couldn't get file.");
  return false;
}
...
void SensorHelper::PrintCalData()
{
  ...
  Logger::debug("PrintCalData() - VL53L4CX_GetCalibrationData status: %s", SensorHelper::ErrorToString(status));

  Logger::debug("==== CALIBERATION DATA ==== ");
  Logger::debug("data->customer.global_config__spad_enables_ref_0: %u", data->customer.global_config__spad_enables_ref_0);
  Logger::debug("data->customer.global_config__spad_enables_ref_1: %u", data->customer.global_config__spad_enables_ref_1);
  Logger::debug("data->customer.global_config__spad_enables_ref_2: %u", data->customer.global_config__spad_enables_ref_2);
  Logger::debug("data->customer.global_config__spad_enables_ref_3: %u", data->customer.global_config__spad_enables_ref_3);
  Logger::debug("data->customer.global_config__spad_enables_ref_4: %u", data->customer.global_config__spad_enables_ref_4);
  Logger::debug("data->customer.global_config__spad_enables_ref_5: %u", data->customer.global_config__spad_enables_ref_5);
  Logger::debug("data->customer.global_config__ref_en_start_select: %u", data->customer.global_config__ref_en_start_select);
  Logger::debug("data->customer.ref_spad_man__num_requested_ref_spads: %u", data->customer.ref_spad_man__num_requested_ref_spads);
  Logger::debug("data->customer.ref_spad_man__ref_location: %u", data->customer.ref_spad_man__ref_location);
  Logger::debug("data->customer.algo__crosstalk_compensation_plane_offset_kcps: %u", data->customer.algo__crosstalk_compensation_plane_offset_kcps);
  Logger::debug("data->customer.algo__crosstalk_compensation_x_plane_gradient_kcps: %d",
                data->customer.algo__crosstalk_compensation_x_plane_gradient_kcps);
  Logger::debug("data->customer.algo__crosstalk_compensation_y_plane_gradient_kcps: %d",
                data->customer.algo__crosstalk_compensation_y_plane_gradient_kcps);
  Logger::debug("data->customer.ref_spad_char__total_rate_target_mcps: %u", data->customer.ref_spad_char__total_rate_target_mcps);
  Logger::debug("data->customer.algo__part_to_part_range_offset_mm: %d", data->customer.algo__part_to_part_range_offset_mm);
  Logger::debug("data->customer.mm_config__inner_offset_mm: %d", data->customer.mm_config__inner_offset_mm);
  Logger::debug("data->customer.mm_config__outer_offset_mm: %d", data->customer.mm_config__outer_offset_mm);
}

bool SensorHelper::RemoveCalib()
{
  if(LittleFS.exists(caliberationfilename)) {
    Logger::debug("Existing caliberation file exists, removing!");
    LittleFS.remove(caliberationfilename);
    return true;
  }
  return false;
...
{
  Logger::info("=== Caliberation 1 - VL53L4CX_PerformRefSpadManagement ===");

  VL53L4CX_Error status;
  status = sensor_vl53l4cx_sat.VL53L4CX_PerformRefSpadManagement();
  Logger::debug("VL53L4CX_PerformRefSpadManagement status: %s", SensorHelper::ErrorToString(status));
...
{
  Logger::debug("=== Caliberation 2 - VL53L4CX_PerformOffsetPerVcselCalibration(%d) ===", distanceInMM);

  if(!SensorHelper::LoadCalData()) {
    Logger::error("PerformCaliberation2() - Failed to SetCalData - make sure you run calibration 1 first.");
    return false;
  }

  VL53L4CX_Error status = sensor_vl53l4cx_sat.VL53L4CX_PerformOffsetPerVcselCalibration(distanceInMM);
  Logger::debug("PerformCaliberation2() - VL53L4CX_PerformOffsetPerVcselCalibration status: %s", SensorHelper::ErrorToString(status));

  SensorHelper::PrintCalData();

  if(!SensorHelper::StoreCalData()) {
    Logger::error("PerformCaliberation2() - Failed to store calibration data");
    return false;
  }
...
{
  Logger::debug("=== Caliberation 3 - VL53L4CX_PerformXTalkCalibration ===");
  if(!SensorHelper::LoadCalData()) {
    Logger::error("PerformCaliberation3() - Failed to SetCalData - make sure you run calibration 1 first.");
    return false;
  }

  VL53L4CX_Error status = sensor_vl53l4cx_sat.VL53L4CX_PerformXTalkCalibration();
  Logger::debug("PerformCaliberation3() - VL53L4CX_PerformXTalkCalibration status: %s", SensorHelper::ErrorToString(status));

  SensorHelper::PrintCalData();

  if(!SensorHelper::StoreCalData()) {
    Logger::error("PerformCaliberation3() - Failed to store calibration data");
    return false;
  }
